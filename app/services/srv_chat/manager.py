...
class ChatManager:
    ...
    @staticmethod
    async def get_chat(db: AsyncSession, chat_id: UUID, user_id: UUID) -> Optional[ChatInfo]:
        """Получает чат по ID."""
        if not (chat := await db.scalar(
    ...
    @staticmethod
    async def get_user_chats(db: AsyncSession, user_id: UUID, limit: int = 50) -> list[ChatInfo]:
    ...
    @staticmethod
    async def update_chat_title(db: AsyncSession, chat_id: UUID, user_id: UUID, title: str) -> Optional[ChatInfo]:
    ...
    @staticmethod
    async def get_chat_with_messages(db: AsyncSession, chat_id: UUID, user_id: UUID) -> Optional[ChatWithMessages]:
    ...
    @staticmethod
    async def add_message(db: AsyncSession, chat_id: UUID, message_request: CreateMessageRequest) -> MessageInfo:
