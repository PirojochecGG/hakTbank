...
async def create_purchase(
    chat_id: UUID, request: CreatePurchaseRequest, user: CurrentUser, db: DBSession
) -> PurchaseResponse:
...
async def update_purchase_status(
    ...
    if not (
        purchase := await PurchasesRouterManager.update_purchase_status(
...
async def analyze_purchase(
    request: AnalyzePurchaseRequest, user: CurrentUser
...
async def delete_purchase(
    chat_id: UUID, purchase_id: UUID, user: CurrentUser, db: DBSession
) -> None:
