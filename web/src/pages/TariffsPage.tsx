  ...
  return rawTariffs
    .map((tariff, index) => {
      const rawLimitations =
        tariff.limitations ?? tariff.limits ?? tariff.features ?? [];
  ...
  const handlePurchase = async (tariff: Tariff) => {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Не удалось создать платёж. Попробуйте еще раз.";
